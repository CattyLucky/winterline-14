...
using Content.Client._WL.PersistentCrafting.UI.Coordinators;
...
public sealed partial class PersistentCraftStationWindow : DefaultWindow
{
    ...
    private IReadOnlyList<string> _visibleBranchIds = Array.Empty<string>();
    ...
    public void UpdateState(PersistentCraftState state, PersistentCraftClientPrototypeCache prototypeCache)
    {
        ...
        _recipes = prototypeCache.AllRecipes
            .Where(recipe => IsBranchVisible(recipe.Branch) && recipe.Placement == null)
            .ToList();
    ...
    private void InitializeBranchContainers(IReadOnlyList<string>? branchIds = null)
    {
        _visibleBranchIds = BuildVisibleBranchIds(branchIds);
        _branchCoordinator.SetVisibleBranches(_visibleBranchIds);

        Branches.RemoveAllChildren();
        _branchContainers.Clear();
        _activeSearchInputsByBranch.Clear();

        for (var index = 0; index < _visibleBranchIds.Count; index++)
        {
            var branchId = _visibleBranchIds[index];
            if (!_branchRegistry.TryGetBranchDefinition(branchId, out var definition))
                continue;
        ...
        _viewModel.LastVisibleBranch = _visibleBranchIds.Count > 0
            ? _visibleBranchIds[0]
            : _branchRegistry.FirstBranchId;
    }

    private IReadOnlyList<PersistentCraftRecipePrototype> GetRecipesForBranch(string branch)
    {
        if (!IsBranchVisible(branch))
            return EmptyRecipes;

        IReadOnlyList<PersistentCraftRecipePrototype> source;
        if (_recipeIndex != null)
        {
            source = _recipeIndex.GetByBranch(branch);
        }
        else
        {
        ...
        var filtered = new List<PersistentCraftRecipePrototype>(source.Count);
    ...
    private List<string> BuildVisibleBranchIds(IReadOnlyList<string>? branchIds)
    {
        var allowed = branchIds is { Count: > 0 }
            ? new HashSet<string>(branchIds)
            : null;
        ...
        for (var i = 0; i < _branchRegistry.OrderedBranchIds.Count; i++)
        {
            ...
            if (allowed != null && !allowed.Contains(branch))
                continue;
    ...
    private bool VisibleBranchesMatch(IReadOnlyList<string> branchIds)
    {
        var visible = BuildVisibleBranchIds(branchIds);
        if (visible.Count != _visibleBranchIds.Count)
            return false;

        for (var i = 0; i < visible.Count; i++)
        {
            if (!string.Equals(visible[i], _visibleBranchIds[i], StringComparison.Ordinal))
                return false;
        }
    ...
    private bool IsBranchVisible(string branch)
    {
        return _visibleBranchIds.Contains(branch);
    }
