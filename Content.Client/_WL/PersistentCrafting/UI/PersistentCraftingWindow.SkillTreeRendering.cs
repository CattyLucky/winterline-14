...
public sealed partial class PersistentCraftingWindow
{
        ...
        string? selectedNodeId)
    {
        ...
        var nodeWidth = PersistentCraftingWindow.BaseTierTreeNodeWidth * _treeZoom;
        var nodeHeight = PersistentCraftingWindow.BaseTierTreeNodeHeight * _treeZoom;
        var horizontalGap = PersistentCraftingWindow.BaseTierTreeHorizontalGap * _treeZoom;
        var verticalGap = PersistentCraftingWindow.BaseTierTreeVerticalGap * _treeZoom;
        var padding = PersistentCraftingWindow.BaseTierTreePadding * _treeZoom;
        var lineThickness = MathF.Max(2f, PersistentCraftingWindow.BaseTierTreeLineThickness * MathF.Sqrt(_treeZoom));
        ...
        float nodeHeight)
    {
        ...
        var button = new ContainerButton
        {
            ...
            {
                BackgroundColor = unlocked ? PersistentCraftingWindow.CardUnlockedBackground : canUnlock ? PersistentCraftingWindow.CardAvailableBackground : PersistentCraftingWindow.CardLockedBackground,
                BorderColor = selected ? PersistentCraftingWindow.SelectedBorder : unlocked ? PersistentCraftingWindow.UnlockedBorder : canUnlock ? accent.WithAlpha(0.6f) : PersistentCraftingWindow.CardBorder,
        ...
        body.AddChild(CreateNodeIcon(node, selected ? PersistentCraftingWindow.SelectedBorder : accent, new Vector2(iconSize, iconSize)));
        ...
        var namePlate = new PanelContainer
        {
            ...
            PanelOverride = new StyleBoxFlat
            {
                BackgroundColor = PersistentCraftingWindow.IconBackground.WithAlpha(0.45f),
                BorderColor = PersistentCraftingWindow.CardBorder.WithAlpha(0.55f),
        ...
        nameLabel.SetMarkup($"[color={PersistentCraftingWindow.HeaderTextColor.ToHex()}]{FormattedMessage.EscapeText(ResolveNodeCardCaption(node))}[/color]");
