...
public sealed partial class PersistentCraftStationWindow
{
        ...
        Color accent)
    {
        var button = new Button
        {
            ...
            StyleBoxOverride = new StyleBoxFlat
            {
                ...
                BorderColor = selected ? accent.WithAlpha(0.72f) : PersistentCraftStationWindow.CardMutedBorder,
    ...
    private Control CreateEmptyRecipeListMessage(string searchText, bool craftableOnly)
    {
        ...
        {
            Text = text,
            FontColorOverride = PersistentCraftStationWindow.MutedText,
        };
    }
    ...
    private PanelContainer CreateEmptyDetailPanel(string searchText, bool craftableOnly)
    {
        var panel = new PanelContainer
        {
            ...
            {
                BackgroundColor = PersistentCraftStationWindow.WindowBackground,
                BorderColor = PersistentCraftStationWindow.CardMutedBorder,
                BorderThickness = new Thickness(1),
            }
        };
        ...
        {
            Text = "?",
            FontColorOverride = PersistentCraftStationWindow.MutedText.WithAlpha(0.4f),
            HorizontalAlignment = Control.HAlignment.Center,
            VerticalAlignment = Control.VAlignment.Center,
        });
        ...
        Color accent)
    {
        ...
        for (var tierIndex = 0; tierIndex < tierGroups.Count; tierIndex++)
        {
            ...
            for (var categoryIndex = 0; categoryIndex < tierGroup.Categories.Count; categoryIndex++)
            {
                ...
                var categoryKey = PersistentCraftStationWindow.BuildCategoryGroupKey(branch, tierGroup.Tier, categoryGroup.CategoryId);
                ...
                for (var subCategoryIndex = 0; subCategoryIndex < categoryGroup.SubCategories.Count; subCategoryIndex++)
                {
                    ...
                    {
                        var subCategoryKey = PersistentCraftStationWindow.BuildSubCategoryGroupKey(branch, tierGroup.Tier, categoryGroup.CategoryId, subCategoryId);
    ...
    private Control CreateSubCategoryHeader(string text, int count, bool collapsed, string subCategoryKey)
    {
        ...
        header.SetData(
            ...
            count,
            collapsed,
            PersistentCraftStationWindow.MutedText,
            PersistentCraftUiTheme.SurfacePanelSoft,
            PersistentCraftStationWindow.CardMutedBorder,
            new Thickness(0),
            4);
    ...
    private Control CreateRecipeListEntry(string branch, PersistentCraftRecipePrototype recipe, bool selected)
    {
        ...
        card.SetData(
            ...
            CanViewRecipe(recipe) ? Color.White : PersistentCraftStationWindow.MutedText,
            PersistentCraftStationWindow.MutedText);
        ...
        _recipeEntryControlsByBranch[branch][recipe.ID] = new PersistentCraftStationWindow.RecipeEntryControls(card.Button, card.IconHost);
        ...
        PersistentCraftBranchState branchState)
    {
        ...
        body.AddChild(CreateSection(
            ...
            $"[color={PersistentCraftStationWindow.DescriptionText.ToHex()}]{FormattedMessage.EscapeText(ResolveRecipeDescription(recipe))}[/color]",
            10));
    ...
    private Control CreateSection(string title, string markupText, int padding)
    {
        ...
        section.SetData(title, markupText, PersistentCraftStationWindow.CardMutedBorder, padding);
        return section;
    }
        ...
        Color accent)
    {
        ...
        {
            BackgroundColor = PersistentCraftStationWindow.IconBackground,
    ...
    private Control CreateRecipeIconContent(PersistentCraftRecipePrototype recipe, Vector2 size)
    {
        if (TryGetRecipeTexture(recipe, out var texture))
        {
            ...
            {
                Texture = texture,
                TextureScale = size.X >= PersistentCraftStationWindow.RecipeIconLargeThreshold
                    ? new Vector2(PersistentCraftStationWindow.RecipeIconScaleLarge, PersistentCraftStationWindow.RecipeIconScaleLarge)
                    : new Vector2(PersistentCraftStationWindow.RecipeIconScaleSmall, PersistentCraftStationWindow.RecipeIconScaleSmall),
