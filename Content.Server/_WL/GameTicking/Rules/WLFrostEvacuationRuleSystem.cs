...
public sealed partial class WLFrostEvacuationRuleSystem : GameRuleSystem<WLFrostEvacuationRuleComponent>
{
        ...
        GameRuleStartedEvent args)
    {
        ...
        component.DepartureMapPosition = Vector2.Zero;
        ...
        float frameTime)
    {
        ...
        if (component.RoundEndAt != null && Timing.CurTime >= component.RoundEndAt.Value)
        {
    ...
    private bool TryLandEvacuation(WLFrostEvacuationRuleComponent component)
    {
        ...
        var departurePosition = position + direction * component.ShuttleDepartureDistance;
        var landingCoords = ToMapCoordinates(worldGrid, position);
        var approachCoords = ToMapCoordinates(worldGrid, approachPosition);
        var departureCoords = ToMapCoordinates(worldGrid, departurePosition);
        ...
        var beacon = Spawn(component.EvacuationBeaconPrototype, new EntityCoordinates(worldGrid, position));
        ...
        Log.Info($"WL frost evacuation shuttle {ToPrettyString(shuttleUid)} inbound to map position {landingCoords.Position}; beacon spawned at world grid position {position}.");
        return true;
    }
    ...
    private void StartDeparture(WLFrostEvacuationRuleComponent component)
    {
        ...
        component.RoundEndAt = Timing.CurTime + component.DepartureRoundEndDelay;
        FinalizeManifest(component);
        ...
        var target = new EntityCoordinates(component.EvacuationMap.Value, component.DepartureMapPosition);
        _shuttle.FTLToCoordinates(
